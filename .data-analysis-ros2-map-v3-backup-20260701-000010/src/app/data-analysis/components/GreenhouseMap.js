  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-slate-400">
            {landmark.observationCount} accepted observation{landmark.observationCount === 1 ? "" : "s"} grouped at this ROS2 map location.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-3 rounded-2xl border border-slate-800 bg-slate-900/60 p-3 text-xs leading-5 text-slate-400">
        ...
        <div className="mt-1">
          The exporter marks these camera-bearing fixed-distance map projections as approximate; they are useful for spatial review, not verified tomato depth coordinates.
        </div>
      </div>
    </div>
  );
  ...
  useEffect(() => {
    ...
    if (layer === "kriging") {
      (spatialSummary?.grid ?? []).forEach((cell) => {
        ...
        const opacity = 0.12 + (1 - clamp(cell.uncertainty ?? 1, 0, 1)) * 0.38;
        context.fillStyle = hexToRgba(maturityColor(cell.value), opacity);
        context.fillRect(center.x - side / 2, center.y - side / 2, side + 0.6, side + 0.6);
      });
    }
    ...
    context.fillText(
      hasRosRaster
        ? `${rasterWidth} × ${rasterHeight} px · ${formatNumber(rasterResolution, 3)} m/px · drag to pan · scroll to zoom`
    ...
    <section className="rounded-[2rem] border border-slate-800 bg-slate-900/65 p-5">
      <div className="flex flex-col gap-5 2xl:flex-row 2xl:items-start 2xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-400">
            The saved ROS2 PGM map is the spatial background. The robot route, current robot pose, cumulative tomato landmarks, and Kriging surface are drawn in the same selected-session map frame.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-5 2xl:grid-cols-[minmax(0,1fr)_330px]">
        <div className="overflow-hidden rounded-3xl border border-slate-800 bg-[#050b14]" ref={wrapperRef}>
          ...
          <canvas
            ref={canvasRef}
            className="block h-[680px] w-full touch-none select-none cursor-grab active:cursor-grabbing"
            role="img"
