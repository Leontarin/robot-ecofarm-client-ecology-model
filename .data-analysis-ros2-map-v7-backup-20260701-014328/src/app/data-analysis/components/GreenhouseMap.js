  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-[10px] leading-4 text-slate-400">
        Grid cell near x {formatNumber(probe.x)} · y {formatNumber(probe.y)} m
      </div>
    </div>
  );
}
  ...
  return (
    <aside className="space-y-3 rounded-3xl border border-slate-800 bg-slate-950/70 p-4">
      <div>
        ...
        <p className="mt-1 text-xs leading-5 text-slate-400">
          Every strong accepted YOLO track remains visible as a marker. Kriging uses locally aggregated support anchors only to avoid numerical instability from near-coincident approximate projections.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/55 p-3 text-xs leading-5 text-slate-400">
        The maturity value is a class-derived index: unripe classes are green/low and ripe classes are red/high. It is not a laboratory ripeness measurement. Hover the map in Kriging mode to inspect the prediction and normalized model uncertainty at a grid cell.
      </div>
    </aside>
  );
}
  ...
  useEffect(() => {
    ...
    if (layer === "kriging") {
      (spatialSummary?.grid ?? []).forEach((cell) => {
        ...
        // High-uncertainty cells stay in the model grid, but visually fade so
        // the map does not imply a precise maturity surface where evidence is weak.
        const opacity = 0.1 + support ** 1.75 * 0.85;
        ...
        if (opacity >= 0.075) {
          ...
          context.strokeStyle = hexToRgba(color, Math.max(0.05, opacity * 0.38));
    ...
    context.fillText(
      hasRosRaster
        ? `${rasterWidth} × ${rasterHeight} px · ${formatNumber(rasterResolution, 3)} m/px · ${layer === "kriging" ? "SLAM raster 50% · hover for estimate · " : ""}drag to pan · scroll to zoom`
    ...
    <section className="rounded-[2rem] border border-slate-800 bg-slate-900/65 p-5">
      <div className="flex flex-col gap-5 2xl:flex-row 2xl:items-start 2xl:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-300">
          ...
          <span className="rounded-full border border-fuchsia-400/30 bg-fuchsia-400/10 px-3 py-1.5 text-fuchsia-100">{layer === "kriging" ? `${spatialSummary?.modelAnchorCount ?? 0} Kriging support anchors · ${samples.length} shown` : "Observed anchors"}</span>
        </div>
      </div>
